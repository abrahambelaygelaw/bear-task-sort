...
export const ConveyorBelt = ({
  ...
  return (
    <div
      className="absolute top-1/2 -translate-y-1/2 w-full h-32 md:h-40 lg:h-52 bg-wood border-y-4 border-wood-light conveyor-belt"
    ...
    >
      <div className="w-full h-full bg-gradient-wood opacity-80 relative overflow-hidden">
        {currentTask && !currentTask.processed && !animatingTask && (
          <div
            ref={conveyorRef}
            className={`absolute top-1/5 -translate-y-1/2 ${isPracticeMode ? 'animate-conveyor-infinite' : 'animate-conveyor'}`}
            key={currentTask.id}
            style={{ touchAction: "none" }}
          >
            <TaskCard 
